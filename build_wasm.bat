...
@set FLAGS=-sMIN_WEBGL_VERSION=2 -sWASM=1 -sMALLOC=emmalloc -sALLOW_MEMORY_GROWTH=1 -Wno-writable-strings -sAUDIO_WORKLET=1 -sWASM_WORKERS=1 -g -gseparate-dwarf=%OUTPUT_DIR%/app_wasm.debug.wasm -s SEPARATE_DWARF_URL=http://localhost:8000/%OUTPUT_DIR%/app_wasm.debug.wasm
