...
@set FLAGS=-sMIN_WEBGL_VERSION=2 -sWASM=1 -sINITIAL_MEMORY=3221225472 -sMALLOC=emmalloc -Wno-writable-strings -sAUDIO_WORKLET=1 -sWASM_WORKERS=1 -g -gseparate-dwarf=%OUTPUT_DIR%/app_wasm.debug.wasm  -sSEPARATE_DWARF_URL=http://localhost:8000/%OUTPUT_DIR%/app_wasm.debug.wasm -pthread -sPTHREAD_POOL_SIZE=4 -sOFFSCREEN_FRAMEBUFFER -sOFFSCREENCANVAS_SUPPORT -lGL
